...
//    }
    ...
    private HashMap<String, String> fillQuestionMap() {
        ...
        map.put("Что такое тестирование?", "Поиск разницы между ожидаемым и фактическим поведением");
        ...
        map.put("Что такое методология разработки?", "Совокупность методов, выполняемых на различных стадиях жизненного цикла ПО");
        map.put("Что такое agile?", "Философия гибкой разработки. \n" + "Плюсы:\n" + "Заказчик может наблюдать результаты\n" + "Минусы:\n" + "Сложно оценить стоимость трудозатрат");
        ...
        map.put("Какие твои любимые инструменты в тестировании?", "Глаза и здравый смысл.\n" + "А ещё postman, devtools, и для работы с базой данных - metabase");
        map.put("В чем разница между авторизацией, аутентификацией, идентификаций, верификацией и валидацией", "Для начала система запрашивает логин, пользователь его указывает, система распознает его как существующий — это идентификация.\n" + "    \n" + "После этого Google просит ввести пароль, пользователь его вводит, и система соглашается, что пользователь, похоже, действительно настоящий, раз пароль совпал, — это аутентификация.\n" + "    \n" + "После этого система предоставит пользователю право читать письма в его почтовом ящике и все в таком духе — это авторизация.\n" + "    \n" + "Верификация — что форма логина и пароля вообще есть\n" + "Валидация — проверка на технические ограничения типа пароль не менее 6 символов");
        ...
        map.put("Что такое тест-кейс?", "Это четкое описание действий, которые необходимо выполнить для проверки функционала");
        ...
        map.put("Что такое TMS?", "Система ведения и хранения тест-кейсов");
        ...
        map.put("Что такое Линуксовая консоль (терминал, гитбаш)?", "Инструмент для управления компьютером и запуском команд без мыши. В том числе используется для работы с гит");
        ...
        //вопросы по SQL
        map.put("SQL: Какие типы баз данных ты знаешь и в чем их отличие?", "Реляционные БД SQL - есть взаимосвязи между базами данных. Храним данные в разных взаимосвязанных таблицах (MySQL, PostgreSQL)\n" + "Нереляционные БД noSQL - есть одна база данных");
        ...
        map.put("Какие основные команды чарльза ты знаешь?", "Breakpoints - останавливает запрос и (или) ответ. В момент остановки их можно редактировать.\n" + "Map local - подставляет в body ответа json с твоего компьютера.\n" + "Map remote - позволяет перенаправлять запрос с клиента в другой бэкенд ( например с тестового окружения в прод)");
        map.put("Что делать, если бэкенд уже передали в тест, а фронт ещё не готов?", "Тестировать бэкенд в постман с использованием АПИ, а тестирование базы данных с помощью SQL");
        map.put("Что делать если фронт уже готов, а бэкенд нет?", "Обговорить с командой, ждать ли бэкенд, или мокать данные (сделать заглушку).\n" + "\n" + "Mock (Заглушка) - предоставление данных как минимум - в плане структуры, эквивалентных тому, что возвратил бы обычный бэкенд. Можно использовать чарльз");
        ...
        map.put("Что такое тонкий клиент?", "Это компьютер или программа клиент который переносит все или большую часть задач по обработке информации на сервер. Примером тонкого клиента может быть браузер. Он тонкий потому, что вся основная бизнес-логика, все вычислительные мощности расположены на компьютере сервера, а клиент простое программное обеспечение, которое помогает отправлять запросы на сервер");
        ...
        map.put("Что такое клиент, а что такое бэкенд?", "Клиент - визуализация. \n" + "Бэкенд - логика");
        map.put("JavaScript, Python, Java, GO, С++ какой из этих языков программирования с большей вероятностью для клиента, а какой для бэкенда?", "JavaScript - в основном фронтенд, но также можно написать бэкенд.\n" + "GO - бэкенд\n" + "Python, C++, Java - бэкенд");
        map.put("Что такое http?", "HTTP — протокол передачи данных, цель которого - передать информацию от клиента на сервер");
        map.put("В чем разница между http и https?", "Http - открытый протокол. используется 80 порт, который визуально не отображается. \n" + "Https - закрытый протокол. используется 443 порт, который отображается в виде серого замочка. Вся передаваемая информация шифруется сайтом");
        ...
        map.put("В чем разница между POST и PUT?", "POST - это отправка новых данных\n" + "PUT - вносит изменения в уже имеющееся на сервере информацию\n" + "\n" + "POST подразумевает, что вы передаете данные в теле запроса, а PUT подразумевает, что вы все передаете в урл. Тела запроса у PUT нет");
        ...
        map.put("Что такое XML?", "Это расширенный язык разметки, который хранит в себе информацию и с помощью него эту информацию можно передавать при общении веб сервисов. Структурной единицей XML являются элементы\n" + "Используются теги");
        ...
        map.put("Какие вкладки в devtools ты знаешь и зачем они нужны?", "\nHeaders - смотрим тип запроса, урл хедера запроса и ответа, статус код ответа\n" + "Network - Тут смотрим все запросы\n" + "Fetch/XHR - фильтр. Только запросы к бэкенду\n" + "Response - body ответа\n" + "Payload - body запроса (если есть)\n" + "Performance - вкладка отображает нагрузку, который создает сайт на компьютер пользователя\n" + "Application - панель, где можно очистить хранилище и кэш, а также управлять Базами данных\n" + "Elements - отображается весь HTML и CSS под открытой страницей. Можно посмотреть и внести исправления в файлы CSS и JS");
        map.put("В чем отличие REST от SOAP?", "REST и SOAP на самом деле не сопоставимы. REST - это архитектурный стиль, SOAP - формат обмена сообщениями. в SOAP вы используете формат soap-xml для запросов и ответов. В REST такого фиксированного формата нет. Вы можете обмениваться сообщениями на основе xml, JSON или любого другого удобного формата");
        map.put("Что такое АПИ?", "Интерфейс для взаимодействия с приложением");
        ...
        map.put("Расскажи, какими функциями в Postman ты пользовался(ась)?", "Создавал(а) коллекции, переменные, импортировал(а) курл, писал(а) автотесты, прогонял(а) все автотесты");
